...
function renderUzRegistry(container) {
    ...
    const rows = units.map(ue => {
        ...
        const burnDate = ue.burnAt ? new Date(ue.burnAt).toLocaleString('ru-RU', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit' }) : '—';
...
function renderTransactionsRegistry(container) {
    ...
    if (txs.length === 0) {
        container.innerHTML = '<p class="registry-placeholder">Реестр пуст — передач ещё не было.</p>';
        return;
    }
    const rows = txs.slice().reverse().map(tx => {
        const date = new Date(tx.timestamp).toLocaleString('ru-RU', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit' });
        const ueList = tx.ueIds ? tx.ueIds.map(id => `№${id}`).join(', ') : '—';
        const msg = tx.message ? `«${tx.message}»` : '—';
        return `<tr>
            <td style="padding:6px;font-family:monospace;font-size:0.8rem">${tx.id.slice(0,10)}…</td>
            <td style="padding:6px">${tx.to || '—'}</td>
            <td style="padding:6px">${ueList}</td>
            <td style="padding:6px;color:var(--text-muted)">${msg}</td>
    ...
    container.innerHTML = `
        <table style="width:100%;border-collapse:collapse;font-size:0.85rem;">
            <thead><tr style="border-bottom:1px solid var(--border-color)">
                ...
                <th style="padding:6px;text-align:left">Кому</th>
                <th style="padding:6px;text-align:left">У.Е.</th>
                <th style="padding:6px;text-align:left">Сообщение</th>
...
function initRegistries() {
    ...
    if (closeUz && uzModal) {
        closeUz.addEventListener('click', () => { uzModal.style.display = 'none'; });
    }
    if (uzModal) {
        uzModal.addEventListener('click', (e) => {
            if (e.target === uzModal) uzModal.style.display = 'none';
        });
    }
    ...
    if (closeTx && txModal) {
        closeTx.addEventListener('click', () => { txModal.style.display = 'none'; });
    }
    if (txModal) {
        ...
        btn.addEventListener('click', () => {
            ...
            if (timeOffset) {
                ...
                if (match) {
                    ...
                    if (status) status.textContent = `Смещение: ${Math.round(window.__testTimeOffset / 60000)} мин`;
                }
                return;
            }
