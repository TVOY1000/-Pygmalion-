...
function calculateBurnAt() {
    ...
    // === ПО КАНОНУ: все У.Е. сгорают в ближайшую полночь +1 день ===
    // Если фаза "Сон" (после 20:00) — сгорание через 24+ часа
    if (phase === 'sleep' || phase === 'impulse') {
        const burnDate = new Date(now);
        burnDate.setDate(burnDate.getDate() + 2);  // +2 дня от текущей даты
        burnDate.setHours(0, 0, 0, 0);
        return burnDate.getTime();
    }

    // Обычный случай (активная фаза) — завтра 00:00
...
function confirmTransfer() {
    ...
    // Закрываем модальное окно + toast «Отправлено!»
