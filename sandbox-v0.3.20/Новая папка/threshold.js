...
// Цветовая схема символов (React-версия)
const CHAR_COLORS = {
  ...
  'Q': 'BLUE', 'W': 'BLUE', 'E': 'BLUE', 'R': 'BLUE', 'T': 'BLUE', 'Y': 'BLUE',
  'U': 'BLUE', 'I': 'BLUE', 'O': 'BLUE', 'P': 'BLUE', 'A': 'BLUE', 'S': 'BLUE',
  'D': 'BLUE', 'F': 'BLUE', 'G': 'BLUE', 'H': 'BLUE', 'J': 'BLUE', 'K': 'BLUE',
  'L': 'BLUE', 'Z': 'BLUE', 'X': 'BLUE', 'C': 'BLUE', 'V': 'BLUE', 'B': 'BLUE',
  'N': 'BLUE', 'M': 'BLUE',
  // Зелёные буквы (специальные)
  '𝕯': 'GREEN'
};
  ...
  document.querySelectorAll('.kb-btn').forEach(btn => {
    btn.addEventListener('click', (event) => {
      ...
      if (action === 'delete') {
        ...
        // Обработка разделённой кнопки
        if (altChar) {
          const rect = btn.getBoundingClientRect();
          const x = event.clientX - rect.left;
          const y = event.clientY - rect.top;

          // Диагональ: верх-право = alt символ
          const useAlt = x > y;
