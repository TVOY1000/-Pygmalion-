...
function renderKeyboard() {
  ...
  const tDouble = I18N.t('th_kb_legend_double');
  const tSplit = I18N.t('th_kb_legend_split');
  const tDrag = I18N.t('th_kb_legend_drag');
  ...
  html += `
    <div class="kb-legend">
      <span class="kb-legend-item">
        <span class="kb-legend-dot green"></span> ${tDouble}
      </span>
      <span class="kb-legend-item">
        <span class="kb-legend-dot split"></span> ${tSplit}
      </span>
      <span class="kb-legend-item">
...
function renderKey(key) {
  const tDel = I18N.t('th_kb_del');
  const tSpace = I18N.t('th_kb_space');

  if (key.type === 'func') {
    if (key.val === 'delete') {
      return `<button class="kb-btn kb-control kb-delete" data-action="delete">${tDel}</button>`;
    }
    if (key.val === 'backspace') {
      return `<button class="kb-btn kb-control kb-backspace" data-action="backspace">←</button>`;
    }
    if (key.val === 'space') {
      return `<button class="kb-btn kb-control kb-space" data-char=" ">${tSpace}</button>`;
    }
  }
...
function confirmOK() {
  ...
  if (length < OK_MIN_LENGTH || length > OK_MAX_LENGTH) {
    alert(I18N.t('th_alert_ok_length', { min: OK_MIN_LENGTH, max: OK_MAX_LENGTH }));
    return;
  }
  ...
  showToast(I18N.t('th_alert_ok_accepted'), 'success');
  setTimeout(() => {
...
function initWaystone() {
  ...
  if (savedOK && getCorrectLength(savedOK) >= OK_MIN_LENGTH) {
    ...
    if (isDevMode) {
      ...
      devNotice.innerHTML = I18N.t('th_dev_notice') + '<b>' + savedOK.slice(0, 10) + '&hellip;</b>';
